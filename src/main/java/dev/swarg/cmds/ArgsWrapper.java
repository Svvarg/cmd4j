...
import dev.swarg.cmds.IArgsWrapper;
import static dev.swarg.cmds.CmdUtil.getCharCount;
import static dev.swarg.cmds.CmdUtil.isNullOrEmpty;
import static dev.swarg.cmds.CmdUtil.stringToBoolean;
import static dev.swarg.cmds.CmdUtil.DEFAULT_EMPTY_STR_ARG;

/**
 * 08-01-21
 * Имплементация обёртки вокруг аргументов IArgWrapper для 
 * удобного взаимодействия с вводимыми аргументами.
...
public class ArgsWrapper implements IArgsWrapper {
     ...
     */
    public int optIndex(String... optNames) {
        int len = argsRemain();
     ...
     * @param defOnKeyExists дефолтное значение, возвращаемое при если ключ
     * существует но его значение  не задано
     * @param optNames
     * @return
     */
    @Override
